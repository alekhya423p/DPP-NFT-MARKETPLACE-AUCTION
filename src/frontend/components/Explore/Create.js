...
const Create = ({web3Handler}) => {
    ...
    async function uploadToIPFS() {
        ...
        if (validate()) {
            if (!name || !description || !price || !image || !category || !fileType) return
            /* first, upload to IPFS */
            const data = JSON.stringify({
                name, description, category ,image,fileType,
                posterFileType,
                posterFileUrl,
                status: 'buy_now',
                created_at: new Date() 
    ...
    async function listNFTForSale() {
        ...
        let transaction = await marketplace.createToken(url, price, { value: listingPrice })
